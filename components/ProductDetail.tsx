...
export default function ProductDetail({ title, description, features, icon: Icon }: ProductDetailProps) {
  return (
    <div className="bg-slate-950 min-h-screen pb-20">
        ...
        <div className="container mx-auto px-6 relative z-10 grid md:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-brand-cyan/10 border border-brand-cyan/20 text-brand-cyan text-sm font-bold mb-6">
              <Star className="w-4 h-4 fill-brand-cyan" /> Salesforce Solution
            </div>
      ...
      <section className="py-24">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-sm font-bold text-brand-cyan tracking-widest mb-2">FEATURES</h2>
            <h3 className="text-4xl font-bold text-white">Why Choose {title}?</h3>
          </div>
      ...
      <section className="py-12 border-t border-slate-900 bg-slate-950">
         <div className="container mx-auto px-6 flex flex-wrap justify-center gap-4">
            {[
              { name: "Salesforce Cloud", href: "/products/salesforce-cloud" },
              { name: "CPQ", href: "/products/cpq" },
              { name: "Billing", href: "/products/billing" },
              { name: "Consumer Goods", href: "/products/consumer-goods" },
              { name: "Revenue Lifecycle", href: "/products/revenue-lifecycle" }
            ].map(link => (
              <Link 
                key={link.href} 
                href={link.href}
                className={`px-6 py-3 rounded-full text-sm font-bold border transition-all ${title.includes(link.name) || (link.name === 'CPQ' && title.includes('CPQ')) ? 'bg-white text-black border-white' : 'border-slate-800 text-slate-400 hover:border-brand-cyan hover:text-brand-cyan'}`}
              >
                {link.name}
              </Link>
            ))}
